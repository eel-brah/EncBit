...
char *get_h_str(char *str);
int get_nmb(char *str);
    ...
    while ((c = getopt (argc, argv, "edfkpHSorwh")) != -1)
    {
        switch (c)
        {
            ...
            case 'h': // help
                ...
                "\n"
                "Options:\n"
                "-h help\n-e encrypt\n-d decrypt\n-ef encrypt a single file\n-df decrypt a single file\n"
...
char *get_str(char *text, int SIZE)
{
    ...
    while (result == NULL)
    {
        ...
        {
            fprintf(stderr, "The input was too long, please try again.\n");
            printf("%s", text);
            char c;
            while ((c = getchar()) != '\n' && c != EOF) {}
...
char *get_h_str(char *str){
    ...
    hstr = (char *)malloc(pn * sizeof(char));
    if(hstr == NULL){
        fprintf(stderr, "malloc: %s\n", strerror(errno));
        return NULL;
    }
    ...
    do
    {
        ...
        while(1){
            ...
            { 
                tmp = (char *)realloc(hstr, ++pn * sizeof(char));
                if(tmp == NULL){
                ...
                hstr = tmp;
                hstr[index++] = c;
                if((flags & 0b10000000) == 0b10000000) printf("%c", c);
                else printf("*");
            }
        }
        hstr[index] = '\0';
