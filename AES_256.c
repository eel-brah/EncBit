...
void AES_256_Encryption(unsigned char *plantext, unsigned char *key){
    ...
    // Encrypt
    aes_encryption(plantext, expandedKey);
}

void AES_256_Decryption(unsigned char *ciphertext, unsigned char *key){
    ...
    // Encrypt
    aes_decryption(ciphertext, expandedKey);
}

void aes_encryption(unsigned char *plantext, unsigned char *key){
    // declare the state and copy 16 bytes to it
    ...
    subBytes(state);
    shiftRows(state);
    addRoundKey(state, key + (16 * (rounds + 1)));

    //  copy the encrypted state
    for(int i = 0; i < 16; i++)
        plantext[i] = state[i];
}
...
void keyExpantion(unsigned char *inputKey, unsigned char *outputKey){
    int origenKeyLen = 32;
    int keyLen = origenKeyLen;
    unsigned char rconint = 1;
    unsigned char four[4];

    // Copy the key to the first 32 bytes of the expanded key
    for(int i = 0; i < origenKeyLen; i++)
        outputKey[i] = inputKey[i];

    while (keyLen < 240){
        // Get the last 4 bytes
        for(int i = 0; i < 4; i++)
                four[i] = outputKey[i + keyLen - 4];

        // For each 32 byte key
        if(keyLen % 32 == 0){
            // RotWord - Routate 4 bytes by 1 byte
            unsigned int *four_tmp = (unsigned int*)four;
            *four_tmp = (*four_tmp >> 8) | (*four_tmp & 0xff) << 24;
            // SubWord - sbox
            for(int i = 0; i < 4; i++)
                four[i] = SBox[four[i]];
            // Rcon - add (^) round constant to the key
            four[0] ^= Rcon(rconint);
            rconint++;
        }
        // For each 16 byte key
        else if(keyLen % 16 == 0){
            // SubWord - sbox
            for(int i = 0; i < 4; i++)
                four[i] = SBox[four[i]];
        ...
        // four ^ [keyLen-origenKeyLen], and store in expandedKey:
        for(int i = 0; i < 4; i++){
            outputKey[keyLen] = outputKey[keyLen - origenKeyLen] ^ four[i];
            keyLen++;
        }
    }
}
...
unsigned char Rcon(int in){
    if(in == 1)
        ...
        if (Rcon(in - 1) < 0x80)
            ...
            return (2 * Rcon(in - 1)) ^ 0x11b;
}

void aes_decryption(unsigned char *ciphertext, unsigned char *key){
    // declare the state and copy 16 bytes to it
    unsigned char state[16];
    for(int i = 0; i < 16; i++)
    ...
    for(int i = 0; i < rounds; i++)
    {
        addRoundKey(state, key + (16 * (rounds-i)));
        invMixColumns(state);
        invShiftRows(state);
        invSubBytes(state);
    }
