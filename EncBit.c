...
int main(int argc, char *argv[]){
    ...
    while ((c = getopt (argc, argv, "edfkpHSorwh")) != -1)
    {
        switch (c)
        {
            ...
            case 'h': // help
                printf(
                    "EncBit - AES-256 Encryption & SHA-256 Hashing Tool\n\n"
                    "Encrypt, decrypt, and hash files, directories, or strings using AES-256 or SHA-256.\n\n"
                    "Usage:\n"
                    "  ./EncBit [OPTION] *ENTITY [KEYFILE]\n\n"
                    "*ENTITY: A file, directory, or string to process.\n"
                    "[KEYFILE]: Required when using the -k option (except for -dSk, which decrypts a string with a keyfile).\n\n"
                    "Options:\n"
                    "  -h  Show this help message.\n"
                    "  -e  Encrypt a file, directory, or string.\n"
                    "  -d  Decrypt a file, directory, or string.\n"
                    ...
                    "  -eS  Encrypt a string.\n"
                    "  -dS  Decrypt a string (input must be in hexadecimal format, without quotes ('')).\n"
                    "      Example: ./EncBit -dS 1D C5 48 93 16 C8 08 6A 21 DA A2 E3 AD 8F 5C 7A\n"
                    "  -ekS  Encrypt a string using a keyfile.\n"
                    "  -p  Enable multi-layer encryption (asks for NMB, which changes the key every 16 bytes).\n"
                    "  -r  Rename the output file instead of appending .bin (for encryption) or keeping the same name (for decryption).\n"
                    "  -o  Keep the original file after encryption or decryption (prevents overwriting).\n"
                    "  -w  Display the password and NMB during input.\n\n"
                    "Examples:\n"
                    "  ./EncBit -e dir               Encrypt a directory.\n"
                    "  ./EncBit -e -f file.txt       Encrypt a single file.\n"
                    "  ./EncBit -d -f file.txt.bin   Decrypt a single file.\n"
                    "  ./EncBit -S 'Hello World'     Hash a string using SHA-256.\n"
                    "  ./EncBit -H file.txt          Hash a file using SHA-256.\n"
                    "  ./EncBit -eS 'Hello World'    Encrypt a string.\n"
                    "  ./EncBit -dS HEX_STRING     Decrypt a hexadecimal-encoded string with no quotes.\n"
                    "  ./EncBit -e -p -f file.txt    Encrypt a file with multi-layer protection.\n\n"
                );
                return 0;
            case '?':
            if (isprint (optopt))
